Voici quelques liens de code sources de pages de l'UBB nous ayant aidé pour faire le scrap :


`Page de Benjamin Tameifuna`
[]()
```
view-source:https://www.ubbrugby.com/equipes/equipe-premiere/effectif/j286-benjamin-tameifuna.html
```
`Page de Boris Palu`
[]()
```
view-source:https://www.ubbrugby.com/equipes/equipe-premiere/effectif/j466-boris-palu.html
```
`Page des classements (Top 14 et Champions Cup)`
[]()
```
view-source:https://www.ubbrugby.com/equipes/equipe-premiere/classement.html
```

[`Page des résultats`](https://www.ubbrugby.com/equipes/equipe-premiere/calendrier-resultats.html)
```
view-source:https://www.ubbrugby.com/equipes/equipe-premiere/calendrier-resultats.html
```
